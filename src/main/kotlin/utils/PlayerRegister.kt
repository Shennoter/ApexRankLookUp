...
fun playerRegister() {
    GlobalEventChannel.parentScope(GlobalScope).subscribeAlways<GroupMessageEvent> { event ->
        if (event.message.content == "/apex" || event.message.content == CustomComm.apex) {
            val gson = Gson()
            val users = gson.fromJson(File("${ApexLookUp.dataFolder}/Users.json").readText(), Users::class.java)
            val qid = sender.id
...
fun playerBonder() {
    GlobalEventChannel.parentScope(GlobalScope).subscribeAlways<GroupMessageEvent> { event ->
        ...
        val patternCustom = "${CustomComm.apexbd}[_a-zA-Z0-9\u4e00-\u9fa5 ]+".toRegex()
                ...
                if (id.matches(finalPattern)) { // 判断是否包含汉字和特殊字符
                    ...
                    if ((qid in users.data.keys) && (users.data[qid]?.contains(id) == true)) {
                        ...
                        if (res.first == 1) { //如果还是不行就报错
                            ...
                            ApexLookUp.logger.error(res.second)
                        }
                        if (res.first == 0) {
                            if(qid in users.data.keys) {
                                users.data[qid]?.add(id)
                                File("${ApexLookUp.dataFolder}/Users.json").writeText(gson.toJson(users))
...
fun playerRegList(){
    GlobalEventChannel.parentScope(GlobalScope).subscribeAlways<GroupMessageEvent> { event ->
        if(event.message.content == "/apexreg" || event.message.content == CustomComm.apexreg){
            val gson = Gson()
            val users = gson.fromJson(File("${ApexLookUp.dataFolder}/Users.json").readText(), Users::class.java)
            val qid = sender.id
            if(qid in users.data.keys || (users.data[qid]?.isNotEmpty() == true)){
                var ids = "已绑定的ID：\n"
                users.data[qid]?.forEach {
                    ids += it + "\n"
                }
                ids = ids.trimEnd('\n')
                subject.sendMessage(message.quote() + ids)
            }
            else{
                subject.sendMessage(message.quote() + "未绑定")
            }
        }
    }
}