...
fun mapReminder(){
    ...
    val groups: GroupReminding =
        Gson().fromJson(File("${ApexLookUp.dataFolder}/Reminder.json").readText(), GroupReminding::class.java)
    ...
    if(requestStr.first == 1) {
        ApexLookUp.logger.error("本次地图轮换监听错误，原因：${requestStr.second}，不用反馈本次错误")
    }
    var res = Gson().fromJson(requestStr.second, ApexResponseMap::class.java)
    val cache = File("${ApexLookUp.dataFolder}/map.time")
    if(!cache.exists()) {
        cache.createNewFile()
    }
    ...
    groups.data.forEach {
        ...
        ApexLookUp.logger.info("已启动对${it}的地图轮换提醒")
    }
    var firstStart = true //第一次启动标志
    mapTask = object : TimerTask() {
        override fun run() {
            requestStr = getRes(url)
            if (requestStr.first == 1) {
                if(Config.extendApiKey.isNotEmpty()){ //如果api过热且config有额外apikey，则使用额外apikey重试
            ...
            if (requestStr.first == 1) {
                ApexLookUp.logger.error("本次地图轮换监听错误，原因：${requestStr.second}，不用报告本次错误")
            }else{
                        ...
                        res = Gson().fromJson(getRes(url).second, ApexResponseMap::class.java)
                            groups.data.forEach {
                                ...
                                if (Config.mode == "pic") { //图片模式
                                    ...
                                    ApexLookUp.logger.info("大逃杀地图已轮换")
                                } else { //文字模式
                                    Bot.instances.forEach { bot ->
                                        bot.getGroup(it!!)?.sendMessage("大逃杀地图已轮换")
                                ...
                                ApexLookUp.logger.info("完成一次对${it}的地图轮换提醒")
                            }
                        }
                    }
                }
            firstStart = false
            ApexLookUp.logger.info("完成一次地图监听")
        }
        }
    }
    mapTimer = Timer()
    mapTimer!!.schedule(mapTask, 0, 30 * 60 * 1000) //每半小时获取一次地图信息
}